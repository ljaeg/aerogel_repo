...
def multi_img_generator(X, Y, answers, seed = 7):
	genX = generator.flow(X, answers, seed = seed)
...
TrainGenerator = multi_img_generator(trainX, trainY, trainAnswers, seed = 13)
...
ValGenerator = multi_img_generator(valX, valY, valAnswers, seed = 192)
...
TestGenerator = multi_img_generator(testX, testY, testAnswers, seed = 21)

#For verbosity, I like to be able to see how it performs on positive samples and negative samples
Pos_TestGen = multi_img_generator(TestYes_X[:200], TestYes_Y[:200], np.ones(200), seed = 3)
Neg_TestGen = multi_img_generator(TestNo_X[:200], TestNo_Y[:200], np.zeros(200), seed = 2)
...
merge = Concatenate()([flat_X, flat_Y])
...
#Create the model
model = Model(inputs = [visible_X, visible_Y], outputs = output)
...
TB = TensorBoard(log_dir = os.path.join(TB_dir, "Mar17", "XandY", str(time())))
